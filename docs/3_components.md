...
### Values (Layer Mask)
- Set how the object would be affected by other systems through the layer mask <br><br> ![](res/21.png)
- **Use Layer Mask** : Set an object to be filtered through engine's layer mask

### Values (Decal)
- **Is Decal** : Set if this object has a decal component or not <br><br> ![](res/22.png)
...
### Values (Emitter)
- **Is Emitter** : Set if this object has an emitter component or not <br><br> Since the engine's emitter particle system is different from blender's, you need to set the one in this **Muramasa Object Data** to create a particle effect within the scene <br><br> ![](res/23.png)
- **Material** : Set the particle's material
- **Mesh** : Set the mesh to be used for the particle's emission starting point
- **Shader Type** : Set the shader type to be used for the particle rendering <br><br> ![](res/24.png)
...
### Values (Script)
- **Script File** : The Lua file that is going to be loaded/run to this object, will pass on Entity ID into the script. Leaving the value empty will not export the scripting function attached to this object. <br><br> ![](res/25.png)

### Values (Object Variables)
- **Object Variables** will store script-ingested variables that can be edited within blender, these values are exposed when the lua script that you've set before explicitly exposes this <br><br> ![](res/26.png)
- **Refresh Object Variables** : Update exposed variables that you can modify for the script in the editor
- On the lua side, you can export the values like below, has to start with `-- MURAMASA_HINT` and then accompanied with the three supported value input: `FLOAT`, `STRING` <br><br>
```
-- MURAMASA_HINT
-- FLOAT Hit_Point
...
- **LOD Mode** : Sets how the mesh LOD is generated <br><br> ![](res/28.png)
...
- **Enable Custom Blend Mode** : Set the material to use custom blend mode that came from Wicked Engine instead of the GLTF standard <br><br> ![](res/31.png)
- **Shading Rate** : Set the material shading rate, you may need latest GPU which supports **Variable Rate Shading** <br><br> ![](res/32.png)
...
- **Collider Shape** : Set the shape of the collider <br><br> ![](res/34.png)
