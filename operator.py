            ...
            if i_collection.redline_prefab.include is True:
                ...
                for i_object in i_collection.all_objects:
                    ...
                    if i_object.instance_collection is not None:
                        ...
                        prefab_instance_restore_list.append(i_object)
                ...
                bpy.ops.export_scene.gltf(filepath=file_collection+"/model.gltf",export_format='GLTF_SEPARATE',export_keep_originals=True,use_selection=True)

                # 4. Restore scene data
                bpy.ops.object.select_all(action='DESELECT')
                for i_object in context_restore['selected_objects']:
                    i_object.select_set(True)
                for i_object in prefab_instance_restore_list:
                    i_object.instance_type = 'COLLECTION'

                # 5. Execute import to engine, wait for completion
                if defer_import is not True:
                    subprocess.run(
                        ["./Dev","-t","SCENE_IMPORT","-i",os.path.relpath(file_collection, content_root)],
                        cwd=binary_root
                    )

        if import_main is True:
            subprocess.run(
                ["./Dev","-t","SCENE_IMPORT","-i",os.path.relpath(file_base+".assetsmith", content_root)],
                cwd=binary_root
            )
...
class REDLINE_INTERLINK_OT_previewasset(Operator):
    ...
    def execute(self, context):
        ...
        subprocess.run(
            ["./Dev","-t","SCENE_PREVIEW","-i",os.path.relpath(file_collection, content_root)],
            cwd=binary_root
        )
